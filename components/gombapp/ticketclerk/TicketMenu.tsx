  ...
  return (
    <>
      <div className="mx-auto grid min-h-0 w-full max-w-[500px] flex-1 grid-cols-2 content-center gap-[15px] overflow-y-auto overflow-x-hidden py-[5px] pb-[100px]">
        {isLoading
          ? Array.from({ length: TICKETS.length }).map((_, i) => (
              <div
                key={`skeleton-${i}`}
              ...
              >
                ...
                <div className="mt-2 h-4 w-1/2 rounded-md bg-gombapp-bg/40" />
              </div>
            ))
          : TICKETS.map((ticket) => {
              const disabled = isTicketDisabled(ticket.name, maxCounts);
              return (
                <button
                  key={ticket.name}
                  className={`flex aspect-square w-full cursor-pointer flex-col items-center justify-center rounded-2xl border-none bg-gombapp-text px-2.5 py-[15px] text-[1.1em] text-gombapp-bg transition-transform duration-100 ease-in-out active:scale-[0.96] ${disabled ? '!active:scale-100 cursor-not-allowed bg-gombapp-btn-disabled opacity-[0.45]' : ''}`.trim()}
