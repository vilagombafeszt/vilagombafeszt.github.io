...
document.querySelectorAll('.dinner-button').forEach(button => {
...
function updateTotalPrice() {
    ...
    orderList.forEach(item => {
        ...
        switch (food) {
            case 'Virsli (db)':
                totalPrice += prices.virsliPrice || 0;
                break;
            case 'Sültkrumpli (adag)':
                totalPrice += prices.sultkrumpliPrice || 0;
                break;
            case 'Szósz 1':
                totalPrice += prices.szosz1Price || 0;
                break;
            case 'Szósz 2':
                totalPrice += prices.szosz2Price || 0;
                break;
            case 'Szósz 3':
                totalPrice += prices.szosz3Price || 0;
                break;
        }
    });
        ...
        if (user) {
            ...
            fetchPrices().then(prices => {
                ...
                newOrders.forEach(food => {
                    ...
                    switch (food) {
                        case 'Virsli (db)':
                            foodPrice = prices.virsliPrice || 0;
                            break;
                        case 'Sültkrumpli (adag)':
                            foodPrice = prices.sultkrumpliPrice || 0;
                            break;
                        case 'Szósz 1':
                            foodPrice = prices.szosz1Price || 0;
                            break;
                        case 'Szósz 2':
                            foodPrice = prices.szosz2Price || 0;
                            break;
                        case 'Szósz 3':
                            foodPrice = prices.szosz3Price || 0;
                            break;
                    }
